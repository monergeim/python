...
class edx(object):
    ...
    def setGrade(self, grade, course="6.01x"):
        ...
        """
        if course in ["6.00x","6.01x","6.02x"]:
            sg = courseInfo(course)
            sg.setGrade(grade)
            #print sg.setGrade(grade)
    ...
    def setPset(self, pset, score, course="6.00x"):
        ...
        """
        if course in ["6.00x","6.01x","6.02x"]:
            sp = courseInfo(course)
            return sp.setPset(pset, score)
